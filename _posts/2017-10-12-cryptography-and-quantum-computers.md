...
[Some scientists proposed](https://en.wikipedia.org/wiki/EPR_paradox) in 1935 that on a very small scale, particles can be in a _superposition_. Quantum computers use particles in superpositions to perform calculations. Bits in a classical computer can only be one or zero, but quantum bits can also be in a superposition: both one and zero at the same time. This can be used to perform computations that are not possible in the non-quantum, classical world.

[Schrödinger](https://en.wikipedia.org/wiki/Erwin_Schr%C3%B6dinger) made superposition more tangible by connecting the particle to a [cat](https://en.wikipedia.org/wiki/Schr%C3%B6dinger%27s_cat) in a closed box. The particle and the cat are connected in some way, such that the cat is either alive or dead, depending on the state of the particle. In a classical world, when you open the box and you see a dead cat, you know that the cat has been dead all along. However, in a quantum world, the cat is in a superposition. It is both alive and dead at the same time until you open the box. When you open the box and look inside, only then the universe determines the fate of the cat.
...
Quantum computers work differently than classical computers, and can be used to perform some calculations much faster than classical computers. However, quantum computers are not faster at everything. Instead, quantum computers can perform different operations than classical computers. This makes it possible to use other algorithms with different timing characteristics. Quantum computers are particularly faster when they can take advantage of superposition to perform calculations on all possible inputs simultaneously. This is called quantum parallelism.

In this post, we are particularly interested on the effect on cryptosystems. That is because with crypto, an increase in speed means a reduction of security. Many cryptosystems are based on the fact that calculating something is hard or takes a long time. This may no longer hold for quantum computers.
...
* Pick two random big primes, _p_ and _q_.
* Multiply these two primes to get the modulus of our keys: _N_ = _p_ × _q_
* Pick some small prime, _e_. This together with the modulus is the public key: (_e_, _N_)
* Calculate the private key (_d_) from the two random big primes (_p_, _q_) and the one small prime (_e_).
...
Assume that we have the public key of someone. That means we have the small prime _e_, and the modulus _N_. If we find which two primes _p_ and _q_ multiplied give the modulus _N_, we can calculate the private key, and we have broken RSA.
...
Shor's algorithm uses quantum computing to find a period of a function, and then uses some more classical math to use that period to factor the modulus into two primes.
...
In RSA, the modulus is published as part of the public key. Unlike the clock example, this is not equal to the period. The modulus is made by multiplying two primes, p and q, together. The order of the function is equal to (_p_-1)(_q_-1). We use our clock math algorithm to found some period that evenly divides (_p_-1)(_q_-1). From this we can calculate p and q, and thus the private key.
