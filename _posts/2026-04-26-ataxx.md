...
**h(n) = count(black\_pieces) - count(white\_pieces)**
...
**h(n) = count(black\_pieces) + a \* pieces\_with\_border(1, black) + b \* pieces\_with\_border(2, black)** \
...
```
score = count(black\_pieces) - count(white\_pieces)
...
function getPossibleMoves(board, player) {
	foreach(empty box) {
		move=find\_one\_normal\_move(board, player);
		if (move) {
			moveList.add(move);
		} else {
			moves=find\_all\_jump\_moves(board, player);
			moveList.add(moves);
		}
	}
	return moveList;
